            ...
            foreach (var property in obj.GetType().GetProperties(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance))
            {
                foreach (var attribute in property.GetCustomAttributes(false))
                {
                    ...
                    if (value != null)
                    {
                        ...
                        {
                            //newUrl = ApplyParameterToUrl(newUrl, JsonPropertyAttribute.PropertyName, String.Format("{0:yyyy/MM/dd}", ((DateTime)value)));
                            newUrl = ApplyParameterToUrl(newUrl, JsonPropertyAttribute.PropertyName, ((DateTime)value).ToString("yyyy/MM/dd", CultureInfo.InvariantCulture));
                        }
                        else
                        {
