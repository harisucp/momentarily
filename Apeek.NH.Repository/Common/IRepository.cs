...
{
    public interface IRepository<T> : ISingletonDependency where T : class
    {
        T Get(object id);
        T Load(object id);
        ...
        IList<T> SaveOrUpdate(IList<T> entities);
        ...
        IQueryable<TF> TableFor<TF>();
        IQueryOver<T, T> QueryOver { get; }
